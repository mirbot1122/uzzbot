...
local function create_group(msg)
  ...
  if not is_admin(msg) then
    return "VocÃª nÃ£o Ã© um Administrador."
  end
...
local function set_description(msg, data)
  ...
  return 'DescriÃ§Ã£o do grupo criada:\n\n'..deskripsi
end
--developed by @i9Dev
local function get_description(msg, data)
  local data_cat = 'description'
  if not data[tostring(msg.to.id)][data_cat] then
    return 'O grupo nÃ£o tem uma descriÃ§Ã£o.'
	end
...
local function export_chat_link_callback(cb_extra, success, result)
  ...
  if success == 0 then
    return send_large_msg(receiver, "NÃ£o Ã© possÃ­vel gerar link para este grupo.")
  end
...
local function get_rules(msg, data)
  local data_cat = 'rules'
  if not data[tostring(msg.to.id)][data_cat] then
    return 'O grupo nÃ£o tem regras.'
	end
...
local function lock_group_name(msg, data)
  ...
  local group_name_lock = data[tostring(msg.to.id)]['settings']['lock_name']
    ...
    return 'O nome do grupo jÃ¡ estÃ¡ bloqueado.'
	else
...
local function unlock_group_name(msg, data)
  ...
  local group_name_lock = data[tostring(msg.to.id)]['settings']['lock_name']
    ...
    return 'O nome do grupo jÃ¡ estÃ¡ desbloqueado.'
	else
...
local function lock_group_member(msg, data)
  ...
  local group_member_lock = data[tostring(msg.to.id)]['settings']['lock_member']
    ...
    return 'Os membros do grupo jÃ¡ estÃ£o bloqueados.'
	else
...
local function unlock_group_member(msg, data)
  ...
  local group_member_lock = data[tostring(msg.to.id)]['settings']['lock_member']
    ...
    return 'Os membros do grupo nÃ£o estÃ£o bloqueados.'
	else
...
local function lock_group_photo(msg, data)
  ...
  local group_photo_lock = data[tostring(msg.to.id)]['settings']['lock_photo']
    ...
    return 'A imagem do grupo jÃ¡ estÃ¡ bloqueada.'
	else
...
local function unlock_group_photo(msg, data)
  ...
  local group_photo_lock = data[tostring(msg.to.id)]['settings']['lock_photo']
    ...
    return 'A imagem do grupo ainda nÃ£o foi bloqueada.'
	else
...
local function show_group_settings(msg, data)
  ...
  local text = "ðŸ”§ ConfiguraÃ§Ãµes do grupo\n\nðŸ” Bloquear nome do grupo: "..settings.lock_name.."\nðŸ” Bloquear membros do grupo: "..settings.lock_member
  return text
end

--lock/unlock spam protection
local function lock_group_spam(msg, data)
  return 'ProteÃ§Ã£o Anti Flood foi habilitado'
end

local function unlock_group_spam(msg, data)
    return 'ProteÃ§Ã£o Anti Flood foi desabilitado'
end
